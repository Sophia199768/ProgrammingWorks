...
public:
    ...
    void UPDATE(std::vector<std::pair<std::string, std::string>>& info) { //
        try {
            for (int line = 0; line < amount_of_line; line++) {
                for (auto& it : info) {
                    if (data[index_data[it.first]].second.primary == 1 and find(data[index_data[it.first]].second.inf.begin(), data[index_data[it.first]].second.inf.end(), it.second)) {
                        throw (Exeption(1));
                        return;
                    }
    ...
    void SELECTWHERE(std::vector<std::string>& fields, std::string& question) { // fields - colomns in answer
        std::vector<Node*> roots;
        print Print;
